...
public class ClientUserAction extends BaseAction {
    ...
    public String signIn() throws ServletException, IOException {
        ...
        if(!StringUtils.isNotEmpty(user_id)||!StringUtils.isNotEmpty(user_password)){
            return "sign";
        }
        ...
        if (!userInfo.getLoginState()) {
            result_info = userInfo.getLoginResult();
    ...
    public void pwd() {
        ...
        String result_info = "";
        if (session.get(USER_INFO) != null && ((TabUserInfo) session.get(USER_INFO)).getUserRole() == 1) {
