...
public class TabRoleDAO {
	...
	public TabRole findById(java.lang.Integer id) {
		log.debug("getting TabRole instance with id: " + id);
		try {
			TabRole instance = (TabRole) getCurrentSession().get("org.jj.model.TabRole", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(TabRole instance) {
		...
		try {
			List results = getCurrentSession().createCriteria("org.jj.model.TabRole").add(Example.create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding TabRole instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from TabRole as model where model." + propertyName + "= ?";
	...
	public TabRole merge(TabRole detachedInstance) {
		...
		try {
			TabRole result = (TabRole) getCurrentSession().merge(detachedInstance);
